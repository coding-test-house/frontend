...
export default function OddEvenGameModal({
  ...
  useEffect(() => {
    ...
    const polling = setInterval(() => {
      // if (gamePhase !== 'result') fetchGameData();
      // if (gamePhase !== 'result') {
      //   fetchGameData();
      // }
  ...
  const displayTimeText =
    gamePhase === 'betting'
      ...
      : gamePhase === 'rolling'
      ? '결과 공개까지'
      : '게임 시작까지'; // 50분 30초~59분 59초
  const displayTimeValue =
    gamePhase === 'betting'
      ? formatTime(timeLeft) // 00:00~49:59 → 베팅 마감까지
      : gamePhase === 'rolling'
      ? formatTime(timeLeft) // 50:00~50:29 → 결과 공개까지
      : formatTime(timeLeft); // 50:30~59:59 → 게임 시작까지
